...
namespace WorldOfTanksBlitzApi.Account.Statistics
{
	public class Statistics
	{
		public List<Tuple<object, object>> frags
		{
			get; private set;
		}
			...
			if (helper.subJsons != null)
			{
				foreach (Json subJson in helper.subJsons)
				{
					try
					{
						...
						{
							case "all":
								this.all = new All(subJson);
								break;
							case "clan":
								this.clan = new Clan(subJson);
								break;
							case "rating":
								this.rating = new Rating(subJson);
								break;
							default:
